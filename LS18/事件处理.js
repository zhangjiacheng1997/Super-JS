...
//3.DOM2级事件处理程序：addEventListener()和removeEventListen()
    ...
    //btn.removeEventListener('click',function,false); 删除事件 通过addEventListener添加的事件只能用此删除
//IE不支持DOM2级事件处理程序
//IE事件处理程序
...
//event.target 获取事件目标，查看事件来自哪个元素,target和this不一定一致,this在冒泡阶段是不断改变的,target是不变的
...
//捕获阶段不执行，DOM2级事件处理设为true才会在捕获阶段执行事件处理程序，默认为false